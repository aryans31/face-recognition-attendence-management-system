...
def getProfile(id):
    ...
    for row in result:
        ...
        print(profile)
    return profile
tym=str(datetime.datetime.now())
def present(id):
    cmmd="""update information set time_exit = %s where id = %s AND time_exit IS NULL"""
    mycursor.execute(cmmd, (tym,id))
    mydb.commit()
    ...
    for(x,y,w,h) in faces:
        ...
        if(conf<70):
            if(profile!=None ):
             ...
             cv2.putText(im,"SUCCESSFUL EXIT", (x,y+h+140),font, 1.5, (0, 225,0),3)
             cv2.putText(im,tym, (50,50),font, 0.5, ( 0,0,0),2)
